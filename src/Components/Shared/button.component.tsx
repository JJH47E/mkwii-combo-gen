...
const mouseOverStart = (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    const {style} = event.target as HTMLButtonElement;
    style.backgroundColor = buttonColors[getRandomInt(buttonColors.length)];
}

const mouseOverEnd = (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    const {style} = event.target as HTMLButtonElement;
...
function Button({ onClick, buttonText }: ButtonProps) {
    return (
        <button style={styles.button}
            ...
            onMouseLeave={event => mouseOverEnd(event)}>{buttonText}</button>
    );
}
